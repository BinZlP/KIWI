<?php
	...
	/*
	$class_query = "select * from class_info_test wher b_class=$b_class";
	$class_query_result = $db->query($class_query);
	//$class_row = $class_query_result -> fetch_assoc();
	foreach($class_query_result as $row){}
	echo $class_row['class_name'].$class_row['day1'].$class_row['day1_time'];
	//echo $b_class;
	*/

	/* 검색 시작 */
	$searchColumn = 'b_title';
	$subString = '';
...
<?php
		...
		//$sql = $sql .' and b_class=$b_class ';
		$sql = $sql . ' order by b_no desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
		$result = $db->query($sql);
	}
?>
<!DOCTYPE html>
...
<body>
	<article class="boardArticle">
		...
		<div id="boardList">
			...
			<div class="btnSet">
				<a href="./write.php?b_class=$b_class" class="btnWrite btn">글쓰기</a>
			</div>
			<div class="paging">
				<?php 
				if(empty($allPost)){
