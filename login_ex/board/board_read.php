...
<head>
    ...
    //데이터 베이스 연결하기
				...
				// Retrun to login page when session doesn't exist
				/*
				session_start();
				...
				*/
				...
				$no = $_GET[no];
    // 글 정보 가져오기
				$result = $db_connect -> query("SELECT * FROM board WHERE id=$id");
				...
				<tr>
					...
					<td width=240 bgcolor=white><?=$row[writer_name]?></td>
				</tr>
				<tr>
					<td width=50 height=20 align=center bgcolor=#EEEEEE>
						날&nbsp;&nbsp;&nbsp;짜</td><td width=240
						bgcolor=white><?=$row[written_date]?></td>
    ...
    // 즉 바로 이전 글 ORDER BY id ASC가 함축됨 즉 오름차순으로 정렬되있음
														$query= $db_connect -> query("SELECT id FROM board WHERE id >'$id' LIMIT 1");
														foreach($query as $prev_id){}
...
$query= $db_connect -> query("SELECT id FROM board WHERE id <$id 
	ORDER BY id DESC LIMIT 1");
foreach($query as $next_id);
...
<?
    // 조회수 업데이트
$result=$db_connect -> query("UPDATE board SET view=view+1 WHERE id=$id");
